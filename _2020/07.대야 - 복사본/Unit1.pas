...
begin
   try
      result := 0;
      if SearchBitmap_map(TransColor_0(bmMsg), TransColor_0(bmp_DY_O_Old)) then
         result := 1
      else
         result := 0
   except
...
//         Bitblt(Y_1.Canvas.Handle, 0, 0, 12, 14, aDC, 989, 737, SRCCOPY);
         ...
         Bitblt(Bmp_MapMain.Canvas.Handle, 0, 0, 20, 10, aDC, 415, 5, SRCCOPY);
         Bitblt(Bmp_CenterMsg.Canvas.Handle, 0, 0, 20, 10, aDC, 415, 5, SRCCOPY);
...
//   {$IFDEF _LOG_} memo1.Lines.Add(FormatDateTime('HH:NN:SS ZZZ ',now)+' ---------Timer_Cap ON!!!'); {$ENDIF}
      ...
      if (Now_Map = map_DY_O_Old) or (Now_Map = map_DY_O_New) then
      begin
         Timer_Cap.Enabled := False;
         ...
         if Chk_Skill.Checked and (ComboBox1.ItemIndex = 2) then
...
begin
   ...
   end else if ((skillCnt mod 2)=0) then
   begin
