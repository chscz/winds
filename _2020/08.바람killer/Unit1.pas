...
//      DateTimePicker1.Time := IncHour(IncMinute(now,3),2);
      DateTimePicker1.Time := IncHour(IncMinute(now,1),0);
      DateTimePicker2.Time := IncHour(IncMinute(now,3),2);
      DateTimePicker3.Time := IncHour(IncMinute(now,3),2);
      DateTimePicker4.Time := IncHour(IncMinute(now,3),2);
end;
...
begin
   ...
   begin
      try
         TerminateProcess(OpenProcess(PROCESS_TERMINATE, BOOL(0), wHandle_1), 0);
         ShowMessage('성공');
      except
         ShowMessage('실패');
      end;
//      try
//         isTerminated := TerminateProcess(wHandle_1, 0);
...
begin
   if FormatDateTime('HH:NN',DateTimePicker2.Time) = FormatDateTime('HH:NN',now) then
   begin
      try
         CloseHandle(wHandle_2);
      finally
         //
      end;
   end;
...
begin
   if FormatDateTime('HH:NN',DateTimePicker3.Time) = FormatDateTime('HH:NN',now) then
   begin
      try
         CloseHandle(wHandle_3);
      finally
         //
      end;
   end;
...
begin
   if FormatDateTime('HH:NN',DateTimePicker4.Time) = FormatDateTime('HH:NN',now) then
   begin
      try
         CloseHandle(wHandle_4);
      finally
         //
      end;
   end;
