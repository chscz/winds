...
//      if (KeyCode = 256) and (PKeyHook.vkCode = VK_F1) then
      if (KeyCode = 256) and (PKeyHook.vkCode = VK_NEXT) then
      begin
//---------------- 마법해제 START
...
//-----------마블
         ...
         keybd_event(VK_NUMPAD1,0,0,0);   //현무 소환
         delay(100);
         keybd_event(VK_MENU, 0, 0, 0);
         keybd_event(VK_DIVIDE,0,0,0);
         keybd_event(VK_MENU, 0, KEYEVENTF_KEYUP, 0);
         keybd_event(VK_NUMPAD7,0,0,0);   //주작 소환
         delay(100);
         keybd_event(VK_MENU, 0, 0, 0);
         keybd_event(VK_DIVIDE,0,0,0);
         keybd_event(VK_MENU, 0, KEYEVENTF_KEYUP, 0);
         ...
         keybd_event(VkKeyScan('f'),0,0,0);
         keybd_event(VK_NUMPAD1,0,0,0);      // 유인
         delay(200);
         keybd_event(VK_DIVIDE,0,0,0);
         keybd_event(VK_NUMPAD7,0,0,0);
         keybd_event(VkKeyScan('f'),0,0,0);
         keybd_event(VK_NUMPAD7,0,0,0);      // 눈쪼기
         delay(200);

         Result := 1;
      end else if (KeyCode = 257) and (PKeyHook.vkCode = VK_NEXT) then
         Result := 1;
      if Result = 0 then
         // 필터링 된 키가 아니라면 계속 진행
