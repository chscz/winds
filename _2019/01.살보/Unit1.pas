...
{      if (PKeyHook.vkCode = VK_INSERT) and (GetAsyncKeyState(VK_CONTROL) < -32766) then
      begin
         ...
         SetWindowText(Handle_1, Form1.WinTitleText1.Text);
      ...
      if (PKeyHook.vkCode = RecFuncKey.Func_Start) and (GetAsyncKeyState(VK_CONTROL) < -32766) then
      begin
//         Form3.Memo1.Lines.Add('시작');
//         Handle_1 := FindWindow(nil, PChar(Form1.WinTitleText1.Text));
{$IFDEF SALBO}
         if (Form1.Cbx_SkillType.ItemIndex in [0,2]) and (Handle_1 <= 0) then
         begin
            ShowMessage('대상이 없음');
         end else
         begin
            Form1.Timer_SALBO.Interval := 500;
            Form1.Timer_SALBO.Enabled  := True;
            Form1.Btn_Set.Enabled      := False;
            Form1.Btn_Res.Enabled      := False;
         end;
{$ENDIF}
...
{$IFDEF SALBO}
         Form1.Timer_SALBO.Enabled := False;
         Form1.Btn_Set.Enabled     := True;
         Form1.Btn_Res.Enabled     := True;
{$ENDIF}
...
//타이틀 캡션, 버튼 처리
procedure TForm1.FormControlClick(Sender: TObject);
begin
   case (Sender as TButton).Tag of
...
begin
   //
   showmessage('Ctrl + '+Cbx_Start.Text+' : 시작'+#13#10 +
...
procedure TForm1.Cbx_CntChange(Sender: TObject);
begin
...
begin
   case Cbx_SkillType.ItemIndex of
      0,1: begin
...
//마법키 제한
procedure TForm1.SkillNum_OneChar(Sender: TObject; var Key: Char);
begin
...
procedure TForm1.Rad_Click(Sender: TObject);
begin
...
procedure TForm1.Timer_SALBOTimer(Sender: TObject);
begin
{$IFDEF SALBO}
   case Cbx_SkillType.ItemIndex of
      0: begin
      ...
      1: begin
         Timer_SALBO.Interval := 22000;
         keybd_event(VK_ESCAPE,0,0,0); // ESC

         if not (EdtSALBO_Skill_JAHYUL.Text = '') then
         begin
            keybd_event(RecSALBO.JAHYUL,0,0,0); // 자혈or소생
            keybd_event(RecSALBO.JAHYUL,0,0,0);
            keybd_event(RecSALBO.JAHYUL,0,0,0);
         end;

         if not (EdtSALBO_Skill_DOBAL.Text = '') then
         begin
            keybd_event(RecSALBO.DOBAL,0,0,0);  // 도발
         end;

         if not (EdtSALBO_Skill_WOONKI.Text = '')  then
         begin
            keybd_event(RecSALBO.WOONKI,0,0,0); // 운기
         end;

         keybd_event(RecSALBO.SALBO,0,0,0);  // 살보
         keybd_event(RecSALBO.SALBO,0,0,0);

         case Flag_SALBO of
            1:
               begin
                  keybd_event(GroupMemNum_1,0,0,0);   //룹번호
                  keybd_event(GroupMemNum_1,0,0,0);
                  if Cbx_GrpNum2.Visible then
                     Inc(Flag_SALBO)
                  else
                     Flag_SALBO := 1;
            ...
            2:
               begin
                  keybd_event(GroupMemNum_2,0,0,0);   //룹번호
                  keybd_event(GroupMemNum_2,0,0,0);
                  if Cbx_GrpNum3.Visible then
                     Inc(Flag_SALBO)
                  else
                     Flag_SALBO := 1;
            ...
            3:
               begin
                  keybd_event(GroupMemNum_3,0,0,0);   //룹번호
                  keybd_event(GroupMemNum_3,0,0,0);
                  if Cbx_GrpNum4.Visible then
                     Inc(Flag_SALBO)
                  else
                     Flag_SALBO := 1;
            ...
            4:
               begin
                  keybd_event(GroupMemNum_4,0,0,0);   //룹번호
                  keybd_event(GroupMemNum_4,0,0,0);
                  Flag_SALBO := 1;
               end;
         end;
...
procedure TForm1.SetNumberEdit(Edit: TWinControl);
var
   defstyle: dWord;
begin
