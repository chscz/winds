...
const
   ...
   map_BMoon_F2_S = 102;  //흑월 2층 심연
   map_BMoon_F2_N = 102;  //흑월 2층 나락
...
//   {$IFDEF _LOG_} memo1.Lines.Add(FormatDateTime('HH:NN:SS ZZZ ',now)+'[Event] :ScreenCapture --- TransColor'); {$ENDIF}
         ...
         Pos_X := {(Check_Num(X_100)*100) +} (Check_Num(X_10)*10) + Check_Num(X_1); //좌표체크
...
begin
   showmessage('▶ 3, 6 : 0.5초에 한번 / 0 : 5초에 한번'+#13#10+
               ...
               '▶ 잔상남는 경우가 있어 '+IntToStr(Timer_BMoon_CtrlR.Interval)+'초마다 Ctrl+R하는데'+#13#10+
...
begin
   try
      if RBtn_Active.Checked then   //활성모드
      begin
         case Now_Map of
            map_BMoon_F0: begin     //흑월입구
   {$IFDEF _LOG_} memo1.Lines.Add(FormatDateTime('HH:NN:SS ZZZ ',now)+'[Event] 엘베타이머--입구'); {$ENDIF}
               if (Pos_X > 12) and (Pos_X < 17) then begin
                  if (Pos_Y > 11) then begin
   //                  memo1.Lines.Add(FormatDateTime('HH:NN:SS ZZZ ',now)+'[Event] :위로');
                     keybd_event(VK_UP, 0, WM_KEYDOWN, 0);
//                     keybd_event(VK_NUMPAD3, 0, WM_KEYDOWN, 0);
                  end else
                  if (Pos_Y = 11) then begin
            ...
            map_BMoon_F1: begin     //어둠일때
               ...
               if (Pos_Y <= 15) then begin
                  ...
                  if (Pos_X = 21) then begin
                     if (Pos_Y > 7) then begin
                        keybd_event(VK_UP, 0, WM_KEYDOWN, 0);
                     end else   //창떳을떄    타이머 끄고 굴이동 후 다시 몰이타이머/엘베타이머 시작
                     if (Pos_Y <= 7) then begin
                        Timer_BMoon_CtrlR.Enabled := False;
                        Timer_BMoon_Elve.Enabled := False;
                        Timer_BMoon_Skill.Enabled := False;
               ...
               if (Pos_Y <= 15) then begin
                  ...
                  if (Pos_X = 21) then begin
                     ...
                     if (Pos_Y <= 7) then begin
                        ...
                        PostMessage(Handle_1, WM_KEYDOWN, VK_NUMPAD8, 0);    //비영
                        Delay(100);
